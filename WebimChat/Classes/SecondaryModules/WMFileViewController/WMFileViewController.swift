...
class WMFileViewController: UIViewController, WKUIDelegate, WKNavigationDelegate {
    ...
    // MARK: - Outlets
    @IBOutlet weak var chatFileHeaderView: ChatFileHeaderView!
    ...
    private func setupLoadingSubiews() {
        chatFileHeaderView.backgroundColor = AppColor.primary3.getColor()
        chatFileHeaderView.delegate = self
        chatFileHeaderView.downloadButton.addTarget(self, action: #selector(saveFile), for: .touchUpInside)
